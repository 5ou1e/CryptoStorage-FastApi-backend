...
from src.application.wallet import (
    CopiedByWalletDTO,
    CopyingWalletDTO,
...
from src.application.wallet.wallet import WalletDTO
from src.infra.db.models.tortoise import Wallet
from src.presentation.api.schemas.common import Pagination
...
class WalletService:
    def __init__(
        ...
        self.wallet_token_repository = wallet_token_repository
        self.swap_repository = swap_repository

    async def get_wallet_related_wallets(self, address: str) -> WalletRelatedWalletsDTO:
        ...
        async def t(token_id):
            async with sem:
                first_buy_activity, first_sell_activity = await asyncio.gather(
                    self.swap_repository.get_first_by_wallet_and_token(
                        wallet_id, token_id, event_type="buy"
                    ),
                    self.swap_repository.get_first_by_wallet_and_token(
                        wallet_id, token_id, event_type="sell"
                    ),
                )
                ...
                neighbor_buy_activities = (
                    await self.swap_repository.get_neighbors_by_token(
                        token_id=token_id,
                ...
                for w_id_, wallet_data in wallets_map.items():
                    fb, fs = wallet_data["buy"], wallet_data["sell"]
                    if not (fb and fs):
                        continue
                    buy_status = classify_block_relation(
                        fb.block_id, first_buy_block_id
                    )
                    sell_status = classify_block_relation(
                        fs.block_id, first_sell_block_id
                    )
        ...
        wallet_tokens = await self.wallet_token_repository.get(
            filter_by={
                "wallet_id": wallet_id,
        ...
        wallets = await self.wallet_repository.get(
            filter_by={
                "id__in": wallet_ids,
        ...
        result = {"copying": [], "copied_by": [], "similar": []}
