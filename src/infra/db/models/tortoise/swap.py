...
class Swap(Model, UUIDIDMixin, TimestampsMixin):
    wallet = fields.ForeignKeyField(
        "models.Wallet", related_name="swaps", on_delete=fields.CASCADE
    )
    token = fields.ForeignKeyField(
        "models.Token", related_name="swaps", on_delete=fields.CASCADE
    )
    ...
    cost_usd = MyDecimalField(max_digits=40, decimal_places=20, null=True, blank=True)
    price_usd = MyDecimalField(max_digits=40, decimal_places=20, null=True, blank=True)
    is_part_of_transaction_with_mt_3_swappers = fields.BooleanField(
        default=False, verbose_name="Является ли свап частью транзакции с 3+ трейдерами"
    )
